    ...
    if(_.isEqual(this.props.fields, nextProps.fields) && _.isEqual(this.props.data, nextProps.data)){
      ...
      fields.map((item)=>{
        if(item.status === 1){
          ...
          }else if(item.type_extends[0].is_fill === undefined){
            values.keys.map((child,i)=>{
              type_extends.push({
                option_name:values.option_name[i] || '',
              })
      ...
      return (
        <div className='form_item' key={k}>
          {getFieldDecorator(`option_name[${k}]`, { 
            ...
            <Input placeholder="请输入" className={voteSign?data.ratio !== undefined ?'item_input':'item_input_checkbox':'item_input_vote'}/>
          )}
          {
            voteSign && [
              <span key={k+2}>
              ...
              <span key={k+1} style={(data.type_extends[k] ? data.type_extends[k].is_fill == 1 : false) ? null : {visibility: 'hidden'} }>
    ...
    return(
      <div>
        <Form layout='vertical'>
          <p className='title'>
            <span className='title_name'>多选控件属性</span>
          ...
          <div className='step3_form'>
            <p className='form_title'>
              <span style={data.ratio == undefined ? {width: 110} : null}>选项文字</span>
              {
                voteSign && [
                  <span key={1}>填空</span>,
                 <span key={2}>是否必填</span>
                ]
              }
              {
                data.ratio !== undefined &&
